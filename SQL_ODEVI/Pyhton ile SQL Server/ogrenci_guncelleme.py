...
class Ui_OBS_Project(object):
    ...
    def retranslateUi(self, OBS_Project):
        ...
        OBS_Project.setWindowTitle(_translate("OBS_Project", "Öğrenci Veri Tabanı Guncelleme"))
        self.label.setText(_translate("OBS_Project", "Öğrenci Veri Tabanı Guncelleme "))
    ...
    def kaydet(self):
        ...
        veriler = (b,c,d,e,f,g,h,i,k,j,m,n,o,p,r,s,t,u,v,y,yy,z,a)
        print(veriler)
        print(len(veriler))
        komut = 'UPDATE [OgrenciVeriTabani].[dbo].[OgrenciTablosu] SET KimlikID=?,Isim=?,Soyisim=?,Addres=?,TelNo=?,EPosta=?,Uyruk=?,Cinsiyet=?,Sehir=?,DogumGun=?,DogumAy=?,DogumYil=?,Fakulte=?,Bolum=?,OgretimTuru=?,OgrenciTuru=?,OkulEposta=?,DGS=?,YatayGecis=?,KayitGun=?,KayitAy=?,KayitYil=? WHERE OgrenciID=?'
        
        #SQL-LITE3
        veriler_db = [(b,c,d,e,f,g,h,i,k,j,m,n,o,p,r,s,t,u,v,y,yy,z,a)]
        try:
            
            im.execute('''UPDATE ogrenci_bilgileri SET KimlikID=?, Isim=?, Soyisim=?,Addres=?,TelNo=?,EPosta=?,Uyruk=?,Cinsiyet=?, Sehir=?,DogumGun=?,DogumAy=?,DogumYil=?,Fakulte=?,Bolum=?,OgretimTuru=?,OgrenciTuru=?,OkulEposta=?, DGS=?, YatayGecis=?, KayitGun=?,KayitAy=?,KayitYil=? WHERE OgrenciID=?''',(b,c,d,e,f,g,h,i,k,j,m,n,o,p,r,s,t,u,v,y,yy,z,a,))
            vt.commit()
